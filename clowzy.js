// Indicolite#2835 tarafından Clowzy için kodlanmıştır.

const Discord = require("discord.js")
const client = new Discord.Client()
const ayarlar = require("./ayarlar.json")
const moment = require("moment") //Indicolite#2835
const fs = require("fs")
...
fs.readdir('./commands/', (err, files) => {
    ...
    console.log(`${files.length} komut yüklenecek.`) //Indicolite#2835
    files.forEach(f => {
...
client.on('message', async message => {
    ...
    if (message.content === '.tag') {
        message.channel.send(`\`${ayarlar.tag}\``) //Indicolite#2835
    }
})
...
client.on("guildMemberAdd", async(member) => {
    let arwSayılar = {
        ...
        "9": `${ayarlar.dokuzEmoji}` //Indicolite#2835
    }
    ...
    const arw = `Ve senin hesabın sunucumuza kayıt olmak için tüm şartları karşılıyor! ${client.emojis.cache.get(ayarlar.yes)}` //Indicolite#2835
    if (arwZaman < 1296000000) {
        arw = `Ve senin hesabın sunucumuza kayıt olmak için daha çok genç! ${client.emojis.cache.get(ayarlar.no)}` //Indicolite#2835
    }

    member.roles.add(ayarlar.kayıtsızRol)
    member.roles.add(ayarlar.kayıtsızRol)
    member.roles.add(ayarlar.kayıtsızRol) //Indicolite#2835
